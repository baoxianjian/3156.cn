/**
 * 3156 PAGE JS
 * @Copyright Copyright 2015, 3156.cn
 * @Descript: 前后台-地区组件
 * @Author	: 358894454@qq.com
 * @Depend	: core.js、jquery.js(1.7 or later), jquery.form, REGION_JSON页面定义，WK.REGION_CONFIG head全局变量
 * $Id$
 */
;(function ( $, window, document, undefined ) {
	var pluginName = 'region',
		...
		arttype_pl = $('<div class="pop_region_list">\
					<ul id="J_region_pop_province" class="cc"></ul>\
					<div class="hr"></div>\
					<ul id="J_region_pop_city" class="cc">\
						<li><span>请选择</span></li>\
	...
	Plugin.prototype = {
			...
			if(region_pop.length) {
				...
				$.ajax({
					...
					success : function(data){
						if(data.state === true) {
							WK.REGION_CONFIG = data.data;
							//修改模板
							$('#J_region_pl').replaceWith(arttype_pl);
		...
		showProvince : function(){
			...
			//循环省数据
			$.each(WK.REGION_CONFIG, function(i, o){
		...
		getCity : function(pid, cid, did){
			...
			var _this = this,
				...
				arr= [],
				data = WK.REGION_CONFIG[pid]['items'];
			...
			if(!data) {
				//没有数据
				pop_city.html('<li><span>请选择</span></li><li><em class="gray">信息不完整，请联系管理员</em></li>');
				return;
			}
		...
		getDistrict : function (data, did){
			...
			if(!data) {
				pop_district.html('<li><span>请选择</span></li><li><em class="gray">信息不完整，请联系管理员</em></li>');
				return;
			}
			...
			wrap.on('click', 'a.J_item', function(e){
				...
				if(!id) {
					...
					if(child == 'city') {
						...
						data = WK.REGION_CONFIG[$('#J_region_pop_province > li.current').data('id')]['items'][id]['items'];
			...
			if(wrap.length) {
				...
				var region_change = wrap.find('a.J_region_change');
					$('button.J_region_pop_ok').off('click').on('click', function(e){
						...
						wrap.find('input.J_areaid').val($('#J_region_pop_city > li.current').data('id'));
						pop.hide();
