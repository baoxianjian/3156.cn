...
class Controller_agent extends Controller_basepage {
   ...
   //	$modid=$this->request['modid'];//1
        ...
        $srow=array('dateline'=>$dateline,'pdt_name'=>$pdt_name,'areas'=>$areas,'cmp_name'=>$cmp_name,'tel'=>$tel,'link_man'=>$link_man,'user_id'=>$user_id,'sa_id'=>$sa_id,'online_state'=>$online_state,'audit_state'=>$audit_state,'is_del'=>$is_del, 'site_id'=>$siteid, 'web_id'=>$webid, 'mod_type'=>$modtype, 'mod_id'=>$modid);  
        ...
        foreach($data['list'] as $key=>$value){
            ...
            $list[$key]['del_url']=$url=SRoute::createUrl('agent/delagent', array('page' => $page,'id'=>$value['id'],'t'=>$value['type']), $this->request);
            $list[$key]['rec_url']=$url=SRoute::createUrl('agent/recagent', array('page' => $page,'id'=>$value['id'],'t'=>$value['type']), $this->request);
	...
	function pageCmtslist($inPath){
		...
		#得到搜索关键字列表
		  ...
		  $data = $mdl_cmt->getListAgentcomments($page, $srow);
			...
			$list[$key]['start_time']=SUtil::formatTime($list[$i]['start_time']);
			$list[$key]['end_time']=SUtil::formatTime($list[$i]['end_time']);
			$list[$key]['del_url']=$url=SRoute::createUrl('agent/delcmts', array('page' => $page,'id'=>$value['id'],'t'=>$value['type']), $this->request);
			$list[$key]['rec_url']=$url=SRoute::createUrl('agent/reccmts', array('page' => $page,'id'=>$value['id'],'t'=>$value['type']), $this->request);
			$list[$key]['cmp_name']=SString::dcutstr($value['cmp_name'],15);
			$list[$key]['pdt_name']=SString::dcutstr($value['pdt_name'],15);
    ...
    function pageAdd(){
	     ...
	     * 备注被动留言
	     */
	    public function pageCheckcmts($inPath){
	    ...
	    public function pageEditagent($inPath){
	    ...
	    public function pageEditcmts($inPath){
