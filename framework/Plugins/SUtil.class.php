...
class SUtil{
    ...
    */
	...
	public static function error($msg, $jumpUrl='', $title='错误提示' ,$wait='' , $TplUrl='', $close=''){
		...
		$self = new self();
		$self->dispatchJump($msg, 0, $config);die;
	}
	...
	private function dispatchJump($msg,$status,$config){
		...
		echo $sTpl->render($config['TplUrl'],$config);die;
	}
	...
	public static function httpFilter($string){
		...
		if(is_array($string)) {
            ...
            $string=trim($string); //update by baoxianjian 11:39 2015/3/29 清除空格
	...
	public static function getColumn($a=array(), $column='id', $isInt=0,$retStr=true)
	{
		...
		if($retStr) $ret = trim(@implode(',',$ret),',');
		return $ret;
	}
	...
	public static function ip2city($ip="") {
		...
		if(!($DataBegin = fread($fd, 4)) || !($DataEnd = fread($fd, 4)) ) return false;
	...
	public static function curlGet($url){
		...
		// 3. 执行并获取HTML文档内容
		$output = curl_exec($ch);
		if($output === false) $curlErr = curl_error($ch);
		// 4. 释放curl句柄
		curl_close($ch);
		if($curlErr)die($curlErr);
		return $output;
	}
	/**
	 * curl模拟POST
	...
	public static function validEmail($email){
		if(strlen($email)>32 || strlen($email)<4) return false;
    ...
    */
    public static function formatTime($time,$style=1)
    {
        ...
        switch($style)
        {
            case 1: $f='Y-m-d';break;
            case 2: $f='H:i:s';break;
            case 3: $f='Y-m-d H:i:s';break;
            default: $f='Y-m-d';break;
        }
        return date($f,$time);
    }
