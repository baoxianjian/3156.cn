 ...
 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.3.1
 */
define ( "OAUTH2_ERROR_INVALID_CLIENT", "invalid client or invalid client_secret" );
 ...
 * Resource servers SHOULD use this
 ...
 * Resource servers SHOULD only use
...
abstract class SOAuth2 {
	 ...
	 *        	
	 * @return TRUE if client credentials are valid, and MUST return FALSE if invalid.
	 *        
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-2.1 @ingroup oauth2_section_2
	 */
	 ...
	 *        	identifier to be check with.
	 *        	
	 * @return Registered redirect URI of corresponding client identifier, and MUST
	 ...
	 *
	 * @return A list as below. If you support all grant types, then you'd do:
	 *         @code
	 *         return array(
	 ...
	 *
	 * @return A list as below. If you support all authorization response types,
	 *         then you'd do:
	 *         @code
	 *         return array(
	 ...
	 *
	 * @return A list as below, for example:
	 *         @code
	 *         return array(
	 *         'my-friends',
	 *         'photos',
	 ...
	 * @param $client_id Client
	 *        	identifier to be check with.
	 * @param $response_type Authorization
	 *        	response type to be check with, would be one of the
	 *        	values contained in OAUTH2_AUTH_RESPONSE_TYPE_REGEXP.
	 *        	
	 * @return TRUE if the authorization response type is supported by this
	 ...
	 *        	type to be check with, would be one of the values contained in
	 ...
	 *         - scope: (optional) Stored scope values in space-separated string.
	 *        
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.1.1 @ingroup oauth2_section_4
	 */
	 ...
	 *         @endcode
	 *        
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.1.2 @ingroup oauth2_section_4
	 */
	 ...
	 * @param $client_id Client
	 *        	identifier to be check with.
	 * @param $assertion_type The
	 *        	format of the assertion as defined by the authorization server.
	 * @param $assertion The
	 *        	assertion.
	 *        	
	 * @return TRUE if the assertion is valid, and FALSE if it isn't. Moreover, if
	 ...
	 *         @endcode
	 *        
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.1.3 @ingroup oauth2_section_4
	 */
	 ...
	 *         - scope: (optional) Stored scope values in space-separated string.
	 *        
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.1.4 @ingroup oauth2_section_4
	 */
	 ...
	 *
	 * @return A string that you want to send in a WWW-Authenticate header.
	 *        
	 ...
	 *
	 * @param $scope A
	 *        	space-separated string of required scope(s), if you want to check
	 *        	for scope.
	 * @param $exit_not_present If
	 *        	TRUE and no access token is provided, send a 401 header and exit,
	 *        	otherwise return FALSE.
	 * @param $exit_invalid If
	 *        	TRUE and the implementation of getAccessToken() returns NULL, exit,
	 *        	otherwise return FALSE.
	 * @param $exit_expired If
	 *        	TRUE and the access token has expired, exit, otherwise return FALSE.
	 * @param $exit_scope If
	 *        	TRUE the access token does not have the required scope(s), exit,
	 *        	otherwise return FALSE.
	 * @param $realm If
	 *        	you want to specify a particular realm for the WWW-Authenticate
	 *        	header, supply it here.
	 *        	
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-5 @ingroup oauth2_section_5
	 */
	 ...
	 *
	 * @param $required_scope Required
	 *        	scope to be check with.
	 * @param $available_scope Available
	 *        	scope to be compare with.
	 *        	
	 ...
	 *
	 * @return Access token value if present, and FALSE if it isn't.
	 *        
	 * @todo Support PUT or DELETE.
	 *      
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-5.1 @ingroup oauth2_section_5
	 */
	private function getAccessTokenParams($exit=false) {
		...
		if ($auth_header !== FALSE) {
			...
			if (isset ( $_GET [OAUTH2_TOKEN_PARAM_NAME] ) || isset ( $_POST [OAUTH2_TOKEN_PARAM_NAME] ))
				return $exit?$this->errorJsonResponse ( OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Auth token found in GET or POST when token present in header' ):FALSE;
			
			$auth_header = trim ( $auth_header );
			
			// Make sure it's Token authorization
			if (strcmp ( substr ( $auth_header, 0, 5 ), "OAuth " ) !== 0)
				return $exit?$this->errorJsonResponse ( OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Auth header found that doesn\'t start with "OAuth"' ):FALSE;
				
				// Parse the rest of the header
			if (preg_match ( '/\s*OAuth\s*="(.+)"/', substr ( $auth_header, 5 ), $matches ) == 0 || count ( $matches ) < 2)
				return $exit?$this->errorJsonResponse ( OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Malformed auth header' ):FALSE;
		...
		if (isset ( $_GET [OAUTH2_TOKEN_PARAM_NAME] )) {
			if (isset ( $_POST [OAUTH2_TOKEN_PARAM_NAME] )) // Both GET and POST are not allowed
				return $exit?$this->errorJsonResponse ( OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Only send the token in GET or POST, not both' ):FALSE;
	 ...
	 *
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4 @ingroup oauth2_section_4
	 */
	public function grantAccessToken() {
		$filters = array (
				"grant_type" => array (
						...
						"options" => array (
								"regexp" => OAUTH2_GRANT_TYPE_REGEXP 
						),
		...
		if (! $input ["scope"])
			$input ["scope"] = NULL;
		$this->setUserid($stored['user_id']);
		$this->setUsername($stored['user_name']);
	 ...
	 *         @endcode
	 *        
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-2 @ingroup oauth2_section_2
	 */
	 ...
	 *        
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-3 @ingroup oauth2_section_3
	 */
	public function getAuthorizeParams() {
		$filters = array (
				"client_id" => array (
						"filter" => FILTER_VALIDATE_REGEXP,
						"options" => array (
								"regexp" => OAUTH2_CLIENT_ID_REGEXP 
						),
						"flags" => FILTER_REQUIRE_SCALAR 
				),
				"response_type" => array (
						"filter" => FILTER_VALIDATE_REGEXP,
						"options" => array (
								"regexp" => OAUTH2_AUTH_RESPONSE_TYPE_REGEXP 
						),
						"flags" => FILTER_REQUIRE_SCALAR 
				),
				"redirect_uri" => array (
	...
	public function finishClientAuthorization($is_authorized, $params = array()) {
		...
		//----params 里含有client_id，response_type，hardware，scope，state，refer，redirect_uri等等信息
		extract ( $params );
		if($params['qymoblie']==1){
			$result ["query"]["qymoblie"] = 1;
		}
		if ($state !== NULL)
	 ...
	 *        	to be pass though buildUri().
	 *        	
	 *        	@ingroup oauth2_section_3
	 */
	private function doRedirectUriCallback($redirect_uri, $params) {
		//---一般情况登录后是query里没有qymoblie这项
	
		if($params['query']['qymoblie']==1){//如果是客户端直接输出json
			unset($params['query']['qymoblie']);
			echo json_encode($params['query']);
			exit();
		}else{
			//--------跳转地址
	...
	protected function createAccessToken($client_id, $scope = NULL) {
		...
		$re = $this->setAccessToken ( $token ["access_token"], $client_id, time () + $this->getVariable ( 'access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME ), $scope );
		if($re==false){
			$this->errorJsonResponse ( OAUTH2_HTTP_BAD_REQUEST, $this->getError('msg') );
		}
	...
	private function createAuthCode($client_id, $redirect_uri, $scope = NULL) {
		...
		$re = $this->setAuthCode ( $code, $client_id, $redirect_uri, time () + $this->getVariable ( 'auth_code_lifetime', OAUTH2_DEFAULT_AUTH_CODE_LIFETIME ), $scope );
		if($re==false){
			$this->errorJsonResponse ( OAUTH2_HTTP_BAD_REQUEST, $this->getError('msg') );
		}
		return $code;
	 ...
	 *
	 * @return The Authorization HTTP header, and FALSE if does not exist.
	 *        
	 ...
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.3 @ingroup oauth2_section_4
	 */
	 ...
	 *        	user-agent to when the end-user authorization step is completed.
	 * @param $error A
	 *        	single error code as described in Section 3.2.1.
	 ...
	 *        	
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-3.2 @ingroup oauth2_error
	 */
	 ...
	 *
	 * @param $http_status_code HTTP
	 *        	status code message as predefined.
	 * @param $error A
	 *        	single error code.
	 * @param $error_description (optional)
	 *        	A human-readable text providing additional information,
	 ...
	 *        	additional information about the error.
	 *        	
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.3 @ingroup oauth2_error
	 */
	 ...
	 *        	status code message as predefined.
	 * @param $realm The
	 *        	"realm" attribute is used to provide the protected resources
	 *        	partition as defined by [RFC2617].
	 * @param $scope A
	 *        	space-delimited list of scope values indicating the required scope
	 *        	of the access token for accessing the requested resource.
	 * @param $error The
	 *        	"error" attribute is used to provide the client with the reason
	 ...
	 *        	resource.
	 *        	
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-5.2 @ingroup oauth2_error
	 */
